...
def mazedisplay(grid,ch,end):	
	...
	for x in zeroes:	
		pygame.draw.rect(screen,white,(x[1]*15,x[0]*15,width,height), thickness)
	for y in ones:
		pygame.draw.rect(screen,black,(y[1]*15,y[0]*15,width,height), thickness)
	if(ch==1 and end==-1):
		twos=(occurrences(grid, 2))
		for z in twos:			
			pygame.draw.rect(screen,red,(z[1]*15,z[0]*15,width,height), thickness)	
	elif(ch==2):
		a,b=end%10,end/10
		pygame.draw.rect(screen,red,(b*15,a*15,width,height), thickness)		
	pygame.display.flip()
...
def dest(a,b):
	...
	time.sleep(10)
def visit(a,b):
	pygame.draw.rect(screen,pink,(b*15,a*15,width,height), thickness)
	pygame.display.update()
	pygame.display.flip()
	pygame.display.update()
def bfspath(path):
